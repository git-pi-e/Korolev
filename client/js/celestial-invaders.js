...
var SPRITE_SHEET_SRC = 'assets/cbimage.png';
var LEFT_KEY = 37;
var RIGHT_KEY = 39;
var SHOOT_KEY = 88;
...
var Player = SheetSprite.extend( {
    ...
    handleInput: function () {
        if ( isKeyDown( LEFT_KEY ) ) this.xVel = -175;
        else if ( isKeyDown( RIGHT_KEY ) ) this.xVel = 175;
        else this.xVel = 0;
    ...
    updateBullets: function ( dt ) {
        for ( var i = this.bullets.length - 1;i >= 0;i-- ) {
            ...
            if ( bullet.alive ) bullet.update( dt );
            else {
...
function drawStartScreen () {
    fillCenteredText( "It seems we have a 404", CANVAS_WIDTH / 2, CANVAS_HEIGHT / 4, '#FFF', 36 );
    fillCenteredText( "Celestial Invaders", CANVAS_WIDTH / 2, CANVAS_HEIGHT / 2.75, '#FFF', 36 );
    fillBlinkingText( "Press Enter to play!", CANVAS_WIDTH / 2, CANVAS_HEIGHT / 2, 500, '#FFF', 36 );
}
...
function animate () {
    ...
    let mainGrad = ctx.createLinearGradient( 0, 0, 0, CANVAS_HEIGHT );
    mainGrad.addColorStop( 0, "#111" );
    mainGrad.addColorStop( 0.5, "#206" );
    mainGrad.addColorStop( 1, "#60a" );
